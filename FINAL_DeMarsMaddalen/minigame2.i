...
void drawMinigame2() {
    ...
    if (instructions2 == 0) {
        ...
        shadowOAM[shadowOAMcounter].attr2 = ((20)*32+(24)) | ((0)<<12);
    }
    shadowOAMcounter++;
}
...
void initPaddle() {
    paddle.width = 16;
    paddle.height = 16;
    paddle.worldCol = (240 / 2) - (paddle.width / 2);
    paddle.worldRow = 160 - paddle.height - 1;
    paddle.aniState = LEFT;
}

void updatePaddle() {
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5))) && paddle.worldCol > 0) {
        paddle.aniState = LEFT;
        paddle.worldCol--;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<4))) && paddle.worldCol < 240 - paddle.width - 1) {
        paddle.aniState = RIGHT;
        paddle.worldCol++;
    }
    if (collision(bead.worldCol, bead.worldRow, bead.width, bead.height, paddle.worldCol, paddle.worldRow, paddle.width, paddle.height) && bead.worldRow <= 160 - (paddle.height + bead.height - 1)) {
        bead.rdel *= -1;
        bounceCount++;
    }
}
...
void drawPaddle() {
    shadowOAM[shadowOAMcounter].attr0 = paddle.worldRow | (0<<14);
    shadowOAM[shadowOAMcounter].attr1 = paddle.worldCol | (1<<14);
    if (paddle.aniState == LEFT) {
        shadowOAM[shadowOAMcounter].attr2 = ((2)*32+(10)) | ((0)<<12);
    } else {
        shadowOAM[shadowOAMcounter].attr2 = ((2)*32+(12)) | ((0)<<12);
    }
    shadowOAMcounter++;
}
...
void updateBead() {
    ...
    if (bead.worldCol < 1 || bead.worldCol >= 240 - bead.width - 1) {
        bead.cdel *= -1;
    }
    if (bead.worldRow > 160 - 1) {
        initMinigame2();
    }
...
void drawBead() {
    shadowOAM[shadowOAMcounter].attr0 = bead.worldRow | (0<<14);
    shadowOAM[shadowOAMcounter].attr1 = bead.worldCol | (1<<14);
    shadowOAM[shadowOAMcounter].attr2 = ((3)*32+(8)) | ((0)<<12);
    shadowOAMcounter++;
}
